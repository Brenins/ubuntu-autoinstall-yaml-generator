...
1. **Advanced IP validation** — Validate individual octets and CIDR range
2. **More profiles** — Docker Host, Kubernetes Node, CI/CD Runner
3. **Themes** — Light theme option
4. **Multiple export** — Generate separate `meta-data` and `user-data`
5. **Curtin integration** — Advanced curtin config support
6. **Visual diff** — Compare two YAMLs side by side
7. **Local persistence** — Save configurations in localStorage
8. **i18n** — Multi-language interface

---

**Generated by Ubuntu Autoinstall YAML Generator**